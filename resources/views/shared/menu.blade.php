<a class="list-group-item" id="nav-home" href="{{ route('home') }}">Home / Announcement</a>
<a class="list-group-item" id="nav-Topics" href="{{ url('/topics.php') }}">Topics</a>
<a class="list-group-item" id="nav-Registration" href="{{ url('/registration.php') }}">Registration</a>
<a class="list-group-item" id="nav-Submission" href="{{ url('/submission.php') }}">Title &amp; Abstract Submission</a>
<a class="list-group-item" id="nav-Accommodation" href="{{ url('/accommodation.php') }}">Accommodation</a>
<a class="list-group-item" id="nav-Transportations" href="{{ url('/transportations.php') }}">Transportations</a>
<a class="list-group-item" id="nav-SocialEvents" href="{{ url('/social_events.php') }}">Social Events</a>
<a class="list-group-item" id="nav-Companion" href="{{ url('/companion_program.php') }}">Companion Program</a>
<a class="list-group-item" id="nav-About_Taiwan" href="{{ url('/about_taiwan.php') }}">Information about Taiwan</a>
<a class="list-group-item" id="nav-Travel_Visa" href="{{ url('/travel_and_visa.php') }}">Travel &amp; Visa Information</a>
<a class="list-group-item" href="http://lecospa.ntu.edu.tw/news-and-activities/photo-gallery/everything-about-gravity/">Conference Photos</a>
<a class="list-group-item" id="nav-ConferenceVideos" href="{{ url('/conference_videos.php') }}">Conference Videos</a>
<a class="list-group-item" id="nav-contact" href="{{ route('contact') }}">Contact Us</a>
<a class="list-group-item" id="nav-Publications" href="{{ url('/publications.php') }}">Publications</a>
<a class="list-group-item" href="http://lecospa.ntu.edu.tw">Back to LeCosPA</a>
